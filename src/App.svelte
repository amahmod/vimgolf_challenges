<script lang="ts">
    ...
    let filtered_challenges: Challenge[] = []

    let completed_challenge_items: CompletedChallengeItem[] = []

    function append_completion_status(challenge: Challenge) {
        const is_completed = completed_challenge_items.find(
            i => i.id === challenge.id
        )
        challenge.completed_at = is_completed
    ...
    function filter_challenges() {
        let temp = challenges.map(append_completion_status)
        ...
        if (sorted_column) {
            temp.sort((a, b) => {
                const val_a =
                    (a[sorted_column as keyof typeof a] as number) || 0
                const val_b =
                    (b[sorted_column as keyof typeof b] as number) || 0
    ...
    async function toggle_complete_status(
        ...
        filtered_challenges = filtered_challenges.map(append_completion_status)
...
<div class="mx-4 my-10 border rounded overflow-x-auto shadow-md sm:rounded-lg">
        ...
        <tbody>
            {#each filtered_challenges as item}
                <tr
                    ...
                    <td class="px-6">{item.active_golfers}</td>
                    <td class="px-6">{item.total_entries}</td>
                    <td class="px-6">{item.lowest_score}</td>
